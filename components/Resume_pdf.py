        ...
        if self.resume.get("certifications"):
            ...
            for cert in self.resume["certifications"]:
                if isinstance(cert, dict):
                    name = str(cert.get("name", "") or "").replace("&", "&amp;").strip()
                    issuer = str(cert.get("issuer", "") or "").replace("&", "&amp;").strip()
                    year = str(cert.get("year", "") or "").strip()
                    url = self._ensure_url(cert.get("credential_url", ""))

                    if not name:
                        continue

                    cert_title_html = f'• &nbsp;<a href="{url}"><b><u>{name}</u></b></a>' if url else f"• &nbsp;<b>{name}</b>"

                    right_parts = []
                    if issuer:
                        right_parts.append(f"<i>{issuer}</i>")
                    if year:
                        right_parts.append(f"<b>{year}</b>")

                    right_text_html = " | ".join(right_parts)

                    data = [
                        [
                            Paragraph(cert_title_html, self.bullet_style),
                            Paragraph(right_text_html, self.body_right_style)
                        ]
                    ]
                    ...
                    clean_cert = cert.replace("&", "&amp;")
                    story.append(Paragraph(f"• &nbsp;{clean_cert}", self.bullet_style))
